...
Validation image colors:
  ...
  - Blue rectangles = label areas (field labels)
    ...
    parser.add_argument(
        "--image", help="Path to page image (PNG) for validation overlay"
    )
    parser.add_argument("--output", help="Path for the output validation image")
    parser.add_argument(
        "--page", type=int, help="Page number for validation image (1-based)"
    )
...
def main():
    ...
    if args.image:
        if not args.output:
            print(
                "Error: --output is required when --image is specified", file=sys.stderr
            )
            sys.exit(1)
        if not args.page:
            print(
                "Error: --page is required when --image is specified", file=sys.stderr
            )
            sys.exit(1)
