...
def generate_booktabs_table(
    df, caption, label, align, highlight_header, alternating_rows
):
...
def parse_args():
    ...
    parser.add_argument("--caption", type=str, default="", help="Table caption text")
    parser.add_argument(
        "--label",
        type=str,
        default="",
...
def main():
    ...
    if args.align:
        if len(args.align) != len(df.columns):
            print(
                f"Error: Alignment string length ({len(args.align)}) must match number of columns ({len(df.columns)})",
                file=sys.stderr,
            )
            sys.exit(1)
        align = args.align
