...
def _ensure_package(pip_name, import_name=None):
    ...
    try:
        ...
        print(
            f":: Package '{import_name}' not found. Installing {pip_name}...",
            file=sys.stderr,
        )
        try:
            ...
            try:
                ...
                print(
                    f"Please install manually: uv pip install {pip_name}",
                    file=sys.stderr,
                )
                sys.exit(1)
    ...
    if "x" in data and "y" in data:
        ...
        if is_multi_series:
            ...
            for i in range(n_series):
                ...
                ax.plot(
                    x, y_stack + y_array[i], linewidth=2, color=colors[i % len(colors)]
                )
                y_stack += y_array[i]
        else:
...
def parse_args():
    ...
    parser.add_argument(
        "chart_type", choices=CHART_TYPES.keys(), help="Type of chart to generate"
    )

    data_group = parser.add_mutually_exclusive_group(required=True)
    data_group.add_argument("--data", type=str, help="Chart data as JSON string")
    data_group.add_argument("--csv", type=str, help="Path to CSV file with chart data")
    ...
    parser.add_argument(
        "--grid", action="store_true", help="Enable grid lines (default: off)"
    )
...
def parse_figsize(figsize_str):
    ...
    try:
        ...
        print(
            f"Error: Invalid figsize format: {figsize_str}. Use format like '8x5'",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        print(
            f"Warning: Style '{args.style}' not found, using default", file=sys.stderr
        )
    ...
    try:
        ...
        result_ax = plot_func(
            data, ax, colors=colors, legend_labels=legend_labels, show_grid=args.grid
        )
